...
static void update(Character *c, int x, int y)
{
	...
	Debug::log("%c goes from (%d, %d) to (%d, %d)\n",
	c->getSymb(), cx, cy, x, y);
...
{	
	...
	while (1)
	{
		...
		if ((t && dungeon->hmap[y][x]!=IMMUTABLE) || (!dungeon->hmap[y][x]))
		{
			break;
		}
		else
