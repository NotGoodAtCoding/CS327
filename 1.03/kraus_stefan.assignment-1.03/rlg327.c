...
typedef struct dungeon {
   ...
   * cells.  We may want a cell structure later, but start a performanace  *
  ...
  //ADDED
  //Map overlay of the distances for monsters.
  uint16_t distance_map[DUNGEON_Y][DUNGEON_X];
} dungeon_t;

//ADDED - simple converter for distances[0-61] to ASCII [0-9][a-z][A-Z]
int distance_to_ascii(int dist){
  return (dist > 61) ? 0         :
         (dist > 35) ? 29 + dist : 
         (dist > 9 ) ? 87 + dist : 48 + dist;
}
//ADDED - simple converter from cell hardness to weight for distance calc
uint8_t hardness_to_weight(uint8_t hardness){
  return hardness == 0 ? 1 : (hardness / 85) +1;
} 
//ADDED - Distance comparator for the heap used in dijkstra_distance
static int32_t distance_cmp(const void *key, const void *with) {
  return ((std_distance_t *) key)->cost - ((std_distance_t *) with)->cost;
}
//ADDED - Implementation of dijkstras to calculate distance from the PC.
static void dijkstra_distance(dungeon_t *d, pair_t start, uint8_t tunneling){

  static std_distance_t distance[DUNGEON_Y][DUNGEON_X], *p;
  ...
  //Distance map must be cleared between calls to avoid artifacts.
  memset(d->distance_map, 0, 
         sizeof(d->distance_map[0][0]) * DUNGEON_Y * DUNGEON_X);

  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      distance[y][x].cost = 256;
    }
  }

  distance[start[dim_y]][start[dim_x]].cost = 0;
 
  heap_init(&h, distance_cmp, NULL);

  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      if (mapxy(x, y) != ter_wall_immutable && (tunneling || !( mapxy(x, y) == ter_wall))) {
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    //For each neighbor in a 3x3 grid
    for(i = -1; i < 2; i++){
      for(j = -1; j < 2; j++){
        //The prospective cost is the current cost plus the scaled hardness of the 
        //cell to be checked
        alt = distance[p->pos[dim_y]][p->pos[dim_x]].cost + 
          hardness_to_weight( d->hardness[p->pos[dim_y] + i][p->pos[dim_x] + j] );
        //Since distances over 61 are not rendered, we do not add the node to the heap 
        // if the cost exceeds that value.
        if(distance[p->pos[dim_y] + i][p->pos[dim_x] + j].hn         && 
           alt < distance[p->pos[dim_y] + i][p->pos[dim_x] + j].cost &&
           alt < 62) {
          distance[p->pos[dim_y] + i][p->pos[dim_x] + j].cost = alt;
          d->distance_map[p->pos[dim_y]+i][p->pos[dim_x]+j] = alt;
          heap_decrease_key_no_replace(&h, distance[p->pos[dim_y]+i][p->pos[dim_x]+ j].hn);
        }
      }
    }
  }
...
static void dijkstra_corridor(dungeon_t *d, pair_t start, pair_t to)
{
  ...
  path[start[dim_y]][start[dim_x]].cost = 0;
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    if ((p->pos[dim_y] == to[dim_y]) && p->pos[dim_x] == to[dim_x]) {
      for (x = to[dim_x], y = to[dim_y];
           (x != start[dim_x]) || (y != start[dim_y]);
           p = &path[y][x], x = p->start[dim_x], y = p->start[dim_y]) {
    ...
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) &&
        (path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost >
         ...
         ((p->pos[dim_x] != start[dim_x]) ? 48  : 0))) {
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost =
        p->cost + (hardnesspair(p->pos) ? hardnesspair(p->pos) : 8) +
         ((p->pos[dim_x] != start[dim_x]) ? 48  : 0);
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].start[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].start[dim_x] = p->pos[dim_x];
    ...
    if ((path[p->pos[dim_y]    ][p->pos[dim_x] - 1].hn) &&
        (path[p->pos[dim_y]    ][p->pos[dim_x] - 1].cost >
         p->cost + (hardnesspair(p->pos) ? hardnesspair(p->pos) : 8) +
         ((p->pos[dim_y] != start[dim_y]) ? 48  : 0))) {
      path[p->pos[dim_y]    ][p->pos[dim_x] - 1].cost =
        p->cost + (hardnesspair(p->pos) ? hardnesspair(p->pos) : 8) +
         ((p->pos[dim_y] != start[dim_y]) ? 48  : 0);
      path[p->pos[dim_y]    ][p->pos[dim_x] - 1].start[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y]    ][p->pos[dim_x] - 1].start[dim_x] = p->pos[dim_x];
 ...
 * corridor.  Random internal points prevent corridors start exiting *
...
//ADDED - helper to render a distance map. Separated for clarity AND
//        reusability of the render_dungeon method.
void render_distance_map(dungeon_t *d, uint16_t map[DUNGEON_Y][DUNGEON_X],  pair_t pc_pos)
{
  pair_t p;
  uint16_t distance =0;
  ...
  for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) {
    for (p[dim_x] = 0; p[dim_x] < DUNGEON_X; p[dim_x]++) {
      distance = map[p[dim_y]][p[dim_x]];
      if(p[dim_y] == pc_pos[dim_y] && p[dim_x] == pc_pos[dim_x]){
        putchar('@');
      }

      else if(distance > 0 && distance < 62) {
        putchar(distance_to_ascii(distance));
      }
...
int main(int argc, char *argv[])
{
  ...
  char *save_file;
  char *pgm_file;
  //ADDED - var to hold PC initial position
  pair_t pc_start;
...
`   * abbreviation after a single dash.  We'll add '--rand' (to     *
   ...
   * start a PGM image, so that I was able to create those more      *
 ...
 if (argc > 1) {
  ...
  //ADDED - PC Starts in the corner of the first room.
  pc_start[dim_x] = d.rooms[0].position[dim_x];
  pc_start[dim_y] = d.rooms[0].position[dim_y];

  //ADDED - generate and render the distance maps. 
  // 0 for non-tunneling, 1 for tunneling
  dijkstra_distance(&d, pc_start, 0);
  render_distance_map(&d, d.distance_map, pc_start);
  dijkstra_distance(&d, pc_start, 1);
  render_distance_map(&d, d.distance_map, pc_start);
