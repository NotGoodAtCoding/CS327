...
void gen_monsters(dungeon_t *d, uint32_t game_turn)
{
  uint32_t i;
  npc *m;
  uint32_t room;
  ...
  for (i = 0; i < d->monster_descriptions.size(); i++) {
    ...
    d->charmap[p[dim_y]][p[dim_x]] = (character *) m;
    ...
    m->characteristics = d->monster_descriptions[i].abilities;
