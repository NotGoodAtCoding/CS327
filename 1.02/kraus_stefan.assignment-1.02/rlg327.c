...
int save_rooms_pos(dungeon_t *d, FILE *file){
...
int save_map(dungeon_t *d, FILE *file){
  pair_t p;

  //same loop structure as rendering the dungeon, minus the border
  for (p[dim_y] = 1; p[dim_y] < DUNGEON_Y -1; p[dim_y]++) {
    for (p[dim_x] = 1; p[dim_x] < DUNGEON_X -1; p[dim_x]++) {
        uint8_t hardness = hardnessxy(p[dim_x], p[dim_y]);
...
uint32_t file_size(dungeon_t *d){
  //Product of the dimensions less the border of immutable wall
  return ( ((DUNGEON_X -2) * (DUNGEON_Y -2)) /*the non-major dungeon array */
    + 6 + 4 + 4 /* File Type + Version + Size number of bytes */  
    + (4 * d->num_rooms) ) /*Number of rooms * bytes per room */ ;
}

int save_dungeon(dungeon_t *d){
  char *home_dir;
  FILE *file;
  char file_type_marker[6] = "RLG327";
  uint32_t file_version = 0;
  uint32_t filesize = file_size(d);
  
  
  if(!(	home_dir = getenv("HOME"))) {
    printf("Home directory not found correctly!\n");
    return 1;
  }

  char *filename = strcat(home_dir, "/.rlg327/");
  //mkdir(filename);
  strcat(filename, "dungeon");

  if(!(file = fopen(filename, "w"))){
    printf("Error saving file, save aborted!\n");
    return 2;
  }
  //File typer marker
  fwrite(&file_type_marker, 1, sizeof (strlen(file_type_marker)), file);

  //version
  fwrite(&file_version, 1, sizeof (file_version), file);

  //the integer size of the file
  fwrite(&filesize, 1, sizeof(filesize), file);
  //write the dungeon map
  save_map(d, file);
  //write all of the room positions
  save_rooms_pos(d, file);
